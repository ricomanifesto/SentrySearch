...
Research and structured report synthesis default to `google/gemini-2.5-flash`; evaluation defaults to `google/gemma-4-31b-it:free`. SentrySearch calls them through OpenRouter's Chat Completions API. Research and synthesis requests are pinned to Google AI Studio with provider fallback disabled. If the free evaluation route is unavailable, SentrySearch retries the paid version of the same judge without that provider pin, so OpenRouter may select another provider. Synthesis uses JSON mode with the full Pydantic and evidence contracts enforced before persistence. Its single correction pass keeps a stable per-report routing session and an explicit cache breakpoint on the unchanged dossier; provider-reported cache tokens are recorded with report performance metrics.
