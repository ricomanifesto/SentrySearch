        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    print(
                        f"DEBUG: Content Enricher rate limit exceeded after {max_retries} attempts"
                    )
                    raise e
                ...
                if hasattr(e, "response") and e.response:
                    ...
                    if retry_after_header:
                        try:
                            ...
                            print(
                                f"DEBUG: Content Enricher API provided retry-after: {retry_after} seconds"
                            )
                ...
                print(
                    f"DEBUG: Content Enricher rate limit hit. Waiting {delay:.1f} seconds before retry {attempt + 2}"
                )
                time.sleep(delay)
...
"""
        ...
        try:
            ...
            return {
                ...
                "keywords": ", ".join(
                    source.ml_techniques + [source.company.lower(), "anomaly detection"]
                ),
                "bm25_terms": ", ".join(
                    source.ml_techniques + [source.company.lower(), "ml", "detection", "analysis"]
                ),
...
class DocumentProcessor:
    ...
    def process_document(self, source: MLPaperSource, content: str) -> List[EnrichedChunk]:
        ...
        for i, chunk_content in enumerate(chunks):
            ...
            chunk.bm25_terms = (
                enrichment.get("bm25_terms", "").split(", ") if enrichment.get("bm25_terms") else []
            )
            chunk.faq_questions = (
                enrichment.get("faq_questions", "").split("|")
                if enrichment.get("faq_questions")
                else []
            )
                ...
                metadatas.append(
                    {
                        ...
                        "faq_questions": (
                            "|".join(chunk.faq_questions) if chunk.faq_questions else ""
                        ),
                    }
                )
...
def main():
    ...
    content_enricher = ContentEnricher(model_client)
    ...
    for query in test_queries:
        ...
        if results:
            ...
            print(
                f"Top match: {top_result['metadata']['company']} - {top_result['metadata']['source_title'][:60]}..."
            )
