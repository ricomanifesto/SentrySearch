...
class WorkersClient:
    ...
    def __init__(self, workers_url: str, timeout: int = 30):
        ...
        self.session.headers.update(
            {"Content-Type": "application/json", "User-Agent": "SentrySearch-Python-Client/1.0"}
        )
    ...
    def hybrid_search(
        ...
        try:
            response = self.session.post(
                f"{self.workers_url}/hybrid-search", json=payload, timeout=self.timeout
            )
    ...
    def vector_search(
        ...
        payload = {"query": query, "maxResults": max_results, "filters": filters or {}}

        try:
            response = self.session.post(
                f"{self.workers_url}/vector-search", json=payload, timeout=self.timeout
            )
            response.raise_for_status()
    ...
    def keyword_search(
        ...
        try:
            response = self.session.post(
                f"{self.workers_url}/keyword-search", json=payload, timeout=self.timeout
            )
            response.raise_for_status()
    ...
    def get_available_companies(self) -> List[str]:
        ...
        try:
            response = self.session.get(
                f"{self.workers_url}/metadata/companies", timeout=self.timeout
            )
            response.raise_for_status()
    ...
    def get_available_techniques(self) -> List[str]:
        ...
        try:
            response = self.session.get(
                f"{self.workers_url}/metadata/techniques", timeout=self.timeout
            )
            response.raise_for_status()
...
class ThreatKnowledgeRetriever:
    ...
    def get_ml_guidance(
        ...
        try:
            ...
            search_results = self.workers_client.hybrid_search(
                queries=optimized_query.optimized_queries, max_results=8, filters=filters
            )
    ...
    def _structure_ml_guidance(
        ...
        search_results: List[WorkersSearchResult],
    ) -> Dict:
    ...
    def _calculate_average_scores(self, search_results: List[WorkersSearchResult]) -> Dict:
        ...
        total_scores = {"vector": 0, "keyword": 0, "hybrid": 0, "applicability": 0}
