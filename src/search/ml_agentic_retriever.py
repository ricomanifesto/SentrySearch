...
Usage:
    retriever = MLAgenticRetriever(model_client, knowledge_base)
...
"""
        ...
        try:
            ...
            fallback_query = (
                f"Machine learning approaches for detecting {threat_characteristics.threat_name}"
            )
    ...
    def _parse_optimization_response(
        self, response: str, threat_characteristics: ThreatCharacteristics
    ) -> OptimizedQuery:
        ...
        for line in lines:
            ...
            if line.startswith("QUERIES:"):
                ...
                ml_focus_areas = [
                    area.strip() for area in line.replace("ML_FOCUS_AREAS:", "").split(",")
                ]
                continue
        ...
        if not queries:
            queries = [
                f"Machine learning detection approaches for {threat_characteristics.threat_name}"
            ]
...
class SourceIdentifier:
    ...
    def identify_relevant_sources(
        self, optimized_query: OptimizedQuery, threat_characteristics: ThreatCharacteristics
    ) -> SourceSelection:
        ...
        for vector in threat_characteristics.attack_vectors:
            for company, expertise in self.company_expertise.items():
                for expert_area in expertise:
                    if any(
                        keyword in expert_area
                        for keyword in [vector, threat_characteristics.threat_type]
                    ):
        ...
        for company, expertise in self.company_expertise.items():
            if any(
                threat_type in expert_area or expert_area in threat_type
                for expert_area in expertise
            ):
                relevant_companies.add(company)
...
class EnhancedRetriever:
    ...
    def retrieve_with_context(
        ...
        for query in optimized_query.optimized_queries:
            ...
            for bm25_result in bm25_search_results:
                ...
                if company in source_selection.relevant_companies:
                    ...
                    ml_result = self._create_ml_result_from_bm25(
                        bm25_result, optimized_query, source_selection
                    )
        ...
        ranked_results = sorted(processed_results, key=lambda x: x.hybrid_score, reverse=True)
        ...
        if trace_exporter:
            ...
            if final_results:
                avg_vector_score = sum(r.relevance_score for r in final_results) / len(
                    final_results
                )
                avg_bm25_score = sum(getattr(r, "bm25_score", 0) for r in final_results) / len(
                    final_results
                )
                avg_hybrid_score = sum(r.hybrid_score for r in final_results) / len(final_results)
                avg_applicability_score = sum(r.applicability_score for r in final_results) / len(
                    final_results
                )
    ...
    def _create_ml_result(
        ...
        paper_techniques = set(
            str(t).strip() for t in ml_techniques_str.split("|") if str(t).strip()
        )
    ...
    def _create_ml_result_from_bm25(
        self,
        ...
        result.hybrid_score = (
            bm25_weight * 0.5 + result.applicability_score * 0.4 + term_match_bonus
        )

        return result

    def _fuse_hybrid_results(
        self, vector_results: List[MLRetrievalResult], bm25_results: List[MLRetrievalResult]
    ) -> List[MLRetrievalResult]:
    ...
    def _create_hybrid_result(
        self, vector_result: MLRetrievalResult, bm25_result: MLRetrievalResult
    ) -> MLRetrievalResult:
        ...
        hybrid_result = MLRetrievalResult(
            ...
            applicability_score=max(
                vector_result.applicability_score, bm25_result.applicability_score
            ),
...
class MLAgenticRetriever:
    ...
    def __init__(self, model_client, knowledge_base_path: str = "./ml_knowledge_base"):
        self.client = model_client
        self.knowledge_base = KnowledgeBaseStorage(knowledge_base_path)

        # Initialize agents
        self.query_optimizer = QueryOptimizer(model_client)
    ...
    def get_ml_guidance(
        self, threat_characteristics: ThreatCharacteristics, trace_exporter=None
    ) -> Dict:
        ...
        try:
            ...
            ml_results = self.enhanced_retriever.retrieve_with_context(
                optimized_query, source_selection, max_results=8, trace_exporter=trace_exporter
            )
    ...
    def get_enhanced_ml_guidance(
        self,
        threat_characteristics: ThreatCharacteristics,
        complete_threat_data: Dict,
        trace_exporter=None,
    ) -> Dict:
        ...
        try:
            ...
            optimized_query = self._optimize_query_with_context(
                threat_characteristics, complete_threat_data
            )
            logger.info(
                f"Generated {len(optimized_query.optimized_queries)} context-enhanced queries"
            )
            ...
            ml_results = self.enhanced_retriever.retrieve_with_context(
                optimized_query,
                source_selection,
                max_results=10,  # More results for enhanced mode
            ...
            return self._create_enhanced_fallback_guidance(
                threat_characteristics, complete_threat_data
            )

    def _structure_ml_guidance(
        self,
        threat_characteristics: ThreatCharacteristics,
        optimized_query: OptimizedQuery,
        source_selection: SourceSelection,
        ml_results: List[MLRetrievalResult],
    ) -> Dict:
...
def _optimize_query_with_context(
    self, threat_characteristics: ThreatCharacteristics, complete_threat_data: Dict
) -> OptimizedQuery:
    ...
    if tech_details := complete_threat_data.get("technicalDetails"):
        if capabilities := tech_details.get("capabilities"):
            ...
            if isinstance(capabilities, list):
                cap_names = [
                    cap.get("name", str(cap)) if isinstance(cap, dict) else str(cap)
    ...
    if c2_data := complete_threat_data.get("commandAndControl"):
        if methods := c2_data.get("communicationMethods"):
            ...
            if isinstance(methods, list):
                protocols = [
                    method.get("protocol", str(method)) if isinstance(method, dict) else str(method)
...
def _structure_enhanced_ml_guidance(
    self,
    threat_characteristics: ThreatCharacteristics,
    optimized_query: OptimizedQuery,
    source_selection: SourceSelection,
    ...
    if tech_details := complete_threat_data.get("technicalDetails"):
        if os_data := tech_details.get("operatingSystems"):
            ...
            if isinstance(os_data, list):
                os_names = [
                    os.get("name", str(os)) if isinstance(os, dict) else str(os)
                    for os in os_data[:2]
                ]
...
def _create_enhanced_fallback_guidance(
    self, threat_characteristics: ThreatCharacteristics, complete_threat_data: Dict
) -> Dict:
...
def main():
    ...
    retriever = MLAgenticRetriever(model_client)
