...
class SectionValidator(RetryingModelRequests):
    ...
    def _extract_json_from_text(self, text: str) -> Optional[dict]:
        ...
        logger.debug("No valid JSON found in %s characters of response text", len(text))
        return None
    ...
    def validate_section(self, section_name: str, content: dict) -> dict:
        ...
        try:
            ...
            logger.warning("Validation failed for %s: %s", section_name, e)
    ...
    def validate_complete_profile(
        ...
        if tool_name:
            ...
            logger.debug(
                f"{len(sections_needing_improvement)} sections need improvement: {sections_needing_improvement}"
            )
                ...
                for i, section_name in enumerate(sections_needing_improvement):
                    ...
                    try:
                        ...
                        if enhanced_content and enhanced_content != profile[section_name]:
                            ...
                            logger.debug(
                                f"Enhanced {section_name} - Score improved from {old_score} to {new_score}"
                            )
                        ...
                        logger.warning("Enhancement failed for %s: %s", section_name, e)
...
Your entire response must be valid JSON that can be directly parsed."""
            ...
            logger.debug(f"Captured {len(sources)} web search sources for {section_name}")
            ...
            if hasattr(response, "content") and response.content:
                ...
                logger.debug(
                    f"Processing {len(response.content)} content blocks for {section_name}"
                )
                ...
                for i, content_block in enumerate(response.content):
                    ...
                    if block_type == "web_search_tool_result":
                        ...
                        logger.debug(f"Found web search tool result")
                    elif block_type == "text":
                        if hasattr(content_block, "text"):
                            text_content = content_block.text
                            logger.debug(f"Text block contains {len(text_content)} characters")
                ...
                if text_blocks:
                    ...
                    logger.debug(f"Using post-tool-result text: {len(response_text)} chars")
                elif response_text:
                    logger.debug(f"Using all text blocks: {len(response_text)} chars")
                else:
                    ...
                    logger.debug(f"Fallback text collection: {len(response_text)} chars")

            response_text = response_text.strip()
            logger.debug(f"Final response text length: {len(response_text)}")

            if response_text:
                logger.debug(
                    "Response text available for %s (%s characters)",
                    section_name,
                    len(response_text),
                )
            else:
                logger.debug(f"No response text found for {section_name}")
            ...
            if enhanced_content:
                ...
                if self._is_content_enhanced(content, enhanced_content):
                    logger.debug(f"Successfully enhanced {section_name} with web search")
                    return enhanced_content
                else:
                    logger.debug(f"No significant enhancement for {section_name}")
                    return content
            else:
                logger.debug(f"No valid JSON in enhancement response for {section_name}")
                # Log the response for debugging but don't fail completely
                if response_text:
                    logger.debug("Enhancement response length: %s", len(response_text))
                return content
...
END ATTESTED EVIDENCE"""
    ...
    def _extract_web_search_sources_from_response(
        ...
        logger.debug(f"Extracting sources for {section_name}")
        ...
        if hasattr(response, "content") and response.content:
            logger.debug(f"Found {len(response.content)} content blocks")
            ...
            for i, content_block in enumerate(response.content):
                ...
                if block_type == "web_search_tool_result":
                    logger.debug(f"Found web_search_tool_result block")

                    # Extract content array from the tool result
                    if hasattr(content_block, "content"):
                        search_content = content_block.content
                        logger.debug(f"Tool result content type: {type(search_content)}")

                        if isinstance(search_content, list):
                            logger.debug(f"Found {len(search_content)} search results")

                            for j, result in enumerate(search_content):
                                result_type = getattr(result, "type", "unknown")
                                logger.debug(f"Search result {j} type: {result_type}")
                                ...
                                if result_type == "web_search_result":
                                    ...
                                    logger.debug("Found a web search result candidate")
                                    ...
                                    if url and title:
                                        ...
                                        logger.debug("Added an attested web source")
                ...
                # Also check for server_tool_use blocks that might contain search queries
                    ...
                    if tool_name_attr == "web_search":
                        if hasattr(content_block, "input"):
                            logger.debug("Found web search query metadata")

        logger.debug(f"Total sources extracted for {section_name}: {len(sources)}")
        return sources
